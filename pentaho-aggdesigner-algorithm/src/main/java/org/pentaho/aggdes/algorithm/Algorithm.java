...
public interface Algorithm extends Component {
   ...
   * called {@link #cancel()} and there is no useful result
   */
  Result run(Schema schema, Map<Parameter, Object> parameterValues,
      Progress progress);
  ...
  enum ParameterEnum implements Parameter {
    TIME_LIMIT_SECONDS("timeLimitSeconds", false, Type.INTEGER,
        "Maximum time, in seconds, to run the algorithm. After this time, the "
            + "algorithm returns the best solution it has found so far."),

    AGGREGATE_LIMIT("aggregateLimit", false, Type.INTEGER,
        "Maximum number of aggregates to create"),

    COST_LIMIT("costLimit", false, Type.DOUBLE,
    ...
    ParameterEnum(String name, boolean required, Type type,
        String description) {
      this.name = name;
