...
public class BitSetPlusTest extends TestCase {
  ...
  private int countSupersetIntersectionExhaustively(
    ...
    int count = 0;
    final long possibleSets = 1L << bitCount;
   ...
   */
  private void checkCountSupersetDiff(int bitCount, int setCount, Random random,
      boolean computeExpected, boolean computeActual) {
    ...
    // on average.
    double density = 1.0 - Math.pow(1.0 - 0.9 / (double) bitCount, setCount);
    ...
    long expected;
    if (computeExpected) {
      expected = countSupersetDiffExhaustively(bitCount, a, bitSets);
    } else {
      expected = 0;
    }
    BigInteger actual;
